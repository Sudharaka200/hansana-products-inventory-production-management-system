...
function SalesTruck() {
    ...
    const handleMarkAsFail = async (orderId) => {
        try {
            await axios.put(``);
            fetchOrders();
            setOrders((prev) => 
                prev.map((order) => 
                     order._id === orderId ? { ...order, status: "Fail" } : order
                )
            );
                            ...
                            {orders.map((order, index) => {
                                ...
                                return (
                                    <tr key={order._id}>
                                        ...
                                        <td className={classes}>
                                            <button className='bg-red-800 pt-2 pb-2 pl-5 pr-5 text-white rounded'
                                                onClick={() => handleMarkAsFail(order._id)}
                                            >Fail</button>
                                        </td>
                                    </tr>
                                );
